...
def get_add_team_data():
    with open("../data/add_team_data.json", "rb") as f:
        list = []
        data = json.load(f)
...
#团队运营人员为空数据
def get_add_team_yunying_is_null():
    with open("../data/add_team_yunying_is_null.json", "rb") as f:
        list = []
        data = json.load(f)
        for i in data.values():
...
#团队日期为空
def get_add_team_date_is_null():
    with open("../data/add_team_date_is_null.json", "rb") as f:
        list = []
        data = json.load(f)
        for i in data.values():
            list.append((i.get("team_name"), i.get("team_name_all"), i.get("team_image"), i.get("team_builder"), i.get("team_yunying"), i.get("team_office_address"), i.get("team_connecter"), i.get("team_phone"), i.get("team_introduce"), i.get("assert_info")))
...
#团队介绍为空
def get_add_team_service_is_null():
    with open("../data/add_team_service_is_null.json", "rb") as f:
        list = []
        data = json.load(f)
        for i in data.values():
            list.append((i.get("team_name"), i.get("team_name_all"), i.get("team_image"), i.get("team_builder"), i.get("team_yunying"), i.get("team_office_address"), i.get("team_connecter"), i.get("team_phone"), i.get("team_introduce"), i.get("assert_info")))
...
class AddTeamCase(unittest.TestCase):
    ...
    @parameterized.expand(get_add_team_data())
    def test_add_team(self, team_name, team_name_all, image_url, team_builder, team_yunying, team_office, team_connecter, team_phone, team_introduce, assert_info):
        ...
        try:
            ...
            self.assertEqual(assert_info, assert_ele.text)
    #
    # @parameterized.expand(get_add_team_yunying_is_null())
    ...
    #         self.assertEqual(assert_info, assert_ele.text)
    #
    # @parameterized.expand(get_add_team_date_is_null())
    ...
    #         self.assertEqual(assert_info, assert_ele.text)
    #
    # @parameterized.expand(get_add_team_service_is_null())
